...
class SingStackProc:
    ...
    def __init__(self, filename):
        ...
        self.stk = data_stack.data(datastruct) # define stack variable
        FileInternalSelection = [(0,0)] # i think this is for selecting ROIs
        plugin = file_plugins.identify(filename) # dont quite know what this is for ...
        self.stack = self.stk
        
        file_plugins.load(filename, self.stk, plugin=plugin,selection=FileInternalSelection)       

    
    #------------------------------------------------------------------------------
    def align_stack(self, stk):
        ...
        for k in range(dims[2]):   
            shifts[k,:], err, phasediff = register_translation(stackcontainer[:,:,int(center)],
                  stackcontainer[:,:,k], 50)
            spectr[:,:,k] = self.ft_matrix_shift(stackcontainer[:,:,k],-shifts[k,0],-shifts[k,1])            
    ...
    def ft_matrix_shift(self, A,dy,dx):
    ...
    #---------------------------------------------------------------------------
    def update_frame(self, iev, stk):
            imdat = stk.absdata[:,:,int(iev)].copy()
            return imdat
        
    #---------------------------------------------------------------------------
    def stack_movie(self, stk):
        fig2 = plt.figure()
        ims = []
    ...
    def deglitch_stack(self, stack, iev):
    ...
    def od_part_stack(self, stk,method):
        ...
        if method=='C':
            ...
            GrayImage = self.mat2_gray(imagebuffer)
            ...
            imagebuffer = np.mean(self.stack.absdata,2)
            GrayImage = self.mat2_gray(imagebuffer)            
